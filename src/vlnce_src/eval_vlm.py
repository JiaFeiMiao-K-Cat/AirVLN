...
from src.common.llm_wrapper import LLMWrapper, LLAMA3
...
Your response should carefully consider the 'actions description', the 'navigation instructions', the 'previous actions', and the scene image if it is provided.
...
'response':"""
    ...
    def plan(self, task_description: str, scene = None, prev_actions: Optional[str] = None):
...
7: MOVE_RIGHT, move right by 5 meters."""
        ...
        response = self.vlm.request(prompt, self.model_name, scene)
...
class VLMEvaluator():
    def __init__(self, vlm: str, observation_space, action_space):
        self.observation_space = observation_space
        self.action_space = action_space

        self.planner = VLMPlanner()
        self.planner.set_model(vlm)
    ...
    def act(self, observations, prev_actions, step=0):
        ...
        for instruction, rgb, prev_action in zip(instructions, rgbs, prev_actions):
            ...
            action = self.planner.plan(task_description=instruction, scene=scene, prev_actions=prev_action)
            try: 
